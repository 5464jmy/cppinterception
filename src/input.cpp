...
namespace interception
{
    ...
    void release(const inputable_t& input)
    {
        ...
        const key_data data = get_key_information(get_key_from_variant(input));
        InterceptionKeyStroke stroke{ static_cast<unsigned short>(data.scan_code), INTERCEPTION_KEY_UP, 0 };
    ...
    {
        InterceptionMouseStroke stroke{ 0, INTERCEPTION_MOUSE_MOVE_RELATIVE, 0, pos.x, pos.y, 0 };
        send_stroke(input_mouse, reinterpret_cast<InterceptionStroke*>(&stroke));
    }
    ...
    void move_mouse_to(const point& to, const curve_params* curve)
    {
        ...
        if (!curve) {
            InterceptionMouseStroke stroke{0, INTERCEPTION_MOUSE_MOVE_RELATIVE, 0, to.x - curr.x, to.y - curr.y, 0};
            send_stroke(input_mouse, reinterpret_cast<InterceptionStroke*>(&stroke));
        } else {
            ...
            for (const auto& point: points) {
                ...
                InterceptionMouseStroke stroke{ 0, INTERCEPTION_MOUSE_MOVE_RELATIVE, 0, rel.x, rel.y, 0 };
                send_stroke(input_mouse, reinterpret_cast<InterceptionStroke*>(&stroke));
