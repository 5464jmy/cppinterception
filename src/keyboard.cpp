...
namespace interception
{
    ...
    std::map<std::string, int32_t> get_vk_map()
    {
        ...
        if (vk_map.empty()) {
            for (const auto& key: keys) {
                if (static_vk_map.find(key) != static_vk_map.end()) {
    ...
    std::string get_key_from_variant(const inputable_t& input) {
        const std::string ret = std::visit(
                [](auto&& arg) -> std::string {
                    using T = std::decay_t<decltype(arg)>;
                    if constexpr (std::is_same_v<T, char>) {
                        return std::string(1, arg);
                    } else if constexpr (std::is_same_v<T, std::reference_wrapper<const std::string>>) {
                        return arg.get();
    ...
    {
        return InterceptionKeyStroke{static_cast<unsigned short>(from_data.scan_code), state,
                                     static_cast<unsigned int>(from_data.extended ? INTERCEPTION_KEY_E0 : 0)};
    }
}
